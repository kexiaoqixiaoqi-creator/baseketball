...
/** Maps Sina status_en values to our internal Game status enum. */
...
/**
 * Syncs the game schedule from Sina Sports for a given date window.
 *
 * For each match in the response:
 *  • Ensure a game_day row exists for that date (status = 'pending' if new)
 *  • Upsert the game row (home_team, away_team, status)
 *  • Maintain ext_id_map for mid → game.id
 */
@Injectable()
export class ScheduleSyncService {
   ...
   * @param span  Number of days to cover (default 1)
   */
  async syncSchedule(date: string, span = 1): Promise<{ games: number }> {
    this.logger.log(`Syncing schedule date=${date} span=${span}…`);

    // Build tid → English team name map for consistent storage with player.team
    ...
    // Group matches by date so we create at most one game_day per date
    ...
    for (const [matchDate, dayMatches] of byDate.entries()) {
      ...
      for (const match of dayMatches) {
        ...
        // Resolve English team names via the tid → name map built above
        ...
        if (internalGameId === null) {
          ...
          // Update status if it changed (e.g., scheduled → in_progress → completed)
