...
/**
 * Syncs per-player season-average statistics (ppg, rpg, apg, spg, bpg, topg)
 * then recomputes salary costs for all players in a single global pass.
 *
 * Called once per game day (07:30) so player costs are always up to date
 * before lineup selection opens.
 */
@Injectable()
export class SeasonStatsSyncService {
  ...
  async syncAllSeasonStats(): Promise<{ updated: number }> {
    ...
    // Get all team mappings so we can iterate by tid
    ...
    // Collect (playerId → StatLine) from every team's stats endpoint
    ...
    for (const { extId: tid } of teamMappings) {
      ...
      for (const sp of statsData.players) {
        ...
        if (internalId === null) continue; // player not yet synced via roster
    ...
    // Compute costs in one global pass
