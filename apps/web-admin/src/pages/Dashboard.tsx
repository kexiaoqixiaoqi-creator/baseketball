...
export function Dashboard() {
  ...
  const cards = [
    { label: 'Players', value: counts.players, link: '/players', color: '#e94560', icon: '🏀' },
    { label: 'Teams', value: counts.teams, link: '/teams', color: '#9b59b6', icon: '🏆' },
    { label: 'Game Days', value: counts.gameDays, link: '/game-days', color: '#27ae60', icon: '📅' },
    { label: 'Rooms', value: counts.rooms, link: '/rooms', color: '#f39c12', icon: '🏠' },
    { label: 'Users', value: counts.users, link: '/users', color: '#3498db', icon: '👥' },
  ];
          ...
          {activeDay ? (
            <div style={{ background: '#131f2e', borderRadius: 10, padding: 20, borderLeft: '3px solid #27ae60' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Link to={`/game-days/${activeDay.id}`} style={{ background: '#27ae60', color: '#fff', padding: '10px 20px', borderRadius: 8, textDecoration: 'none', fontWeight: 600, fontSize: 14 }}>
                  Manage →
                </Link>
              </div>
            </div>
