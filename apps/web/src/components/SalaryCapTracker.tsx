  ...
  return (
    <div style={{ background: '#16213e', borderRadius: 8, padding: 16, marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
        <span style={{ color: '#aaa' }}>已用：<strong style={{ color: '#fff' }}>${totalCost.toLocaleString()}</strong></span>
        <span style={{ color: remaining < 0 ? '#e94560' : '#27ae60' }}>
          {remaining < 0 ? `超标：$${Math.abs(remaining).toLocaleString()}` : `剩余：$${remaining.toLocaleString()}`}
        </span>
      </div>
    </div>
  );
